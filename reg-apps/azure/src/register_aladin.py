"""Worker script for affine registration on an Azure ML compute node.

Called by the ``register_aladin`` component defined in ``pipeline.py``.
Each invocation registers one floating image to the reference using
``reg_aladin`` via *niftyregw*.
"""
...
def main(
    ...
    output_affine: Path = typer.Option(..., help="Output affine matrix."),
    output_result: Path = typer.Option(..., help="Output resampled image."),
    input_affine: Optional[Path] = typer.Option(None, help="Initial affine."),
    ...
    rigid_only: bool = typer.Option(False, help="Rigid registration only."),
    extra_args: str = typer.Option("", help="Extra reg_aladin arguments."),
    save_result: bool = typer.Option(False, help="Save resampled image."),
) -> None:
    """Register a floating image to a reference using reg_aladin."""
    args: list[str] = []
    ...
    if input_affine is not None:
        args.extend(["-inaff", str(input_affine)])
